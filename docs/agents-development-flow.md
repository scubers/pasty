...
**⚠️ 重要：开发过程中忽略 LSP (Language Server Protocol) 的错误，以编译脚本的错误为准！**

LSP 工具（如 clangd、VSCode 语言扩展等）可能因项目配置复杂、跨平台特性或解析限制而产生误报。这在 C++、Swift、Kotlin 等语言中都很常见。

**正确做法（适用于所有语言）：**
1. 始终运行对应的编译/构建脚本进行验证
2. 如果 LSP 报告错误但编译脚本通过，则忽略 LSP 错误
3. 如果编译脚本报告错误，则必须修复
4. 不应为了消除 LSP 错误而添加不符合项目规范的修复（如添加不必要的 include 路径或类型转换）
